...
public void setCalendar(int n) {
	...
	GregorianCalendar temp = new GregorianCalendar(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH)+n);
	calendar = temp;
}
public void setDate(int n) {
	GregorianCalendar temp = new GregorianCalendar(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), n);
	calendar = temp;
}
public boolean timeConflict(Event event) {
	String date = event.getEventDate();
	 if(exist(date)==true)
	 {
		 ArrayList<Event> events =  myCalendar.get(date);
		 
		 for (int i = 0 ; i< events.size(); i++){
			 if(events.get(i).getEventStartingTime().equals(event.getEventStartingTime())){
				 return true;
			 }
		 }
		 return false;
